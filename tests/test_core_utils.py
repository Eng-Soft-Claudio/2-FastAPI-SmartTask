...
# =====================================================
# Testes que verificam a lógica de cálculo da pontuação de prioridade.
...
def test_calculate_priority_score_with_invalid_importance_returns_none():
    ...
    # Importância abaixo do mínimo (0)
...
def test_calculate_priority_score_with_no_due_date():
    ...
    # Cenário com importância máxima (5)
    importance_5 = 5
...
def test_calculate_priority_score_with_due_date_today():
    ...
    O fator de data de entrega (`days_to_due`) deve ser 1 (ou o mínimo considerado),
    maximizando a contribuição da data para a pontuação.
    """
    print("\nTeste: calculate_priority_score com data de entrega HOJE (2025-05-04).")
    due_date_is_today = date.today() 
    test_importance = 4
    
    # Assumindo que `days_to_due` = 1 para tarefas com entrega hoje na fórmula.
...
def test_calculate_priority_score_for_overdue_task():
    ...
    Espera-se que a pontuação seja o `PRIORITY_SCORE_IF_OVERDUE` mais o peso da importância.
    """
...
# =====================================================
# Testes que verificam a lógica de identificação de tarefas urgentes.

def _create_dummy_test_task(**kwargs) -> Task:
    """
    Função auxiliar para criar instâncias de `Task` para os testes de `is_task_urgent`.
    Preenche valores base e permite sobrescrevê-los com `kwargs`.
    Se `priority_score` não for fornecido em `kwargs` e `importance` ou `due_date` estiverem,
    a pontuação é calculada usando `calculate_priority_score`.
    """
    ...
    # Calcula priority_score se não for fornecido explicitamente
    # e se houver dados para calculá-lo (importance ou due_date).
    ...
    # Adiciona um try-except para Pydantic ValidationError para depuração.
    try:
...
def test_is_task_urgent_when_no_score_and_no_due_date():
    ...
    # `_create_dummy_test_task` sem `priority_score` ou `due_date` (se não houver importance) resulta em score=None.
    # Ou podemos forçar, passando `importance=0` (ou fora do range) para garantir score=None se `calculate_priority_score`
    # for chamada pela factory.
    task_no_urgency_factors = _create_dummy_test_task(importance=3, priority_score=None, due_date=None) 
    # Para garantir priority_score = None, precisamos ou passar `importance` inválido, ou setar diretamente.
...
def test_is_task_urgent_when_due_date_is_today():
    ...
    A iminência da data de entrega deve torná-la urgente.
    """
...
def test_is_task_urgent_when_score_is_exactly_at_threshold_and_due_date_is_future():
    ...
    A lógica de urgência por score geralmente usa "maior que" (`>`), então esta tarefa
    NÃO deve ser considerada urgente apenas pelo score.
    """
    ...
    # Verificação opcional se o _create_dummy_test_task recalcularia o score.
    if task_at_threshold.priority_score != score_at_threshold:
        print(f"  AVISO: Score da tarefa foi recalculado para {task_at_threshold.priority_score} "
              ...
              f"Verificar lógica de _create_dummy_test_task ou setar priority_score após criação.")
        # Para o teste ser válido com o score exato, podemos forçá-lo:
    ...
    # Assumindo que a urgência por score é `score > threshold`.
    assert is_urgent_result is False, \
        f"Tarefa com score exatamente no limiar ({task_at_threshold.priority_score}) e entrega futura não deveria ser urgente."
